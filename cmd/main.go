...
func main() {
	...
	startTime := time.Now()
	outputImage := MedianFilterParallel(inputImage, windowsSize, numCPU)
	endTime := time.Now()
...
func SaveImage(outputFilepath string, outputImage image.Image) {
...
func ReadImage(inputFilepath string) image.Image {
...
func MedianFilterParallel(input image.Image, windowSize, numThreads int) image.Image {
	bounds := input.Bounds()
